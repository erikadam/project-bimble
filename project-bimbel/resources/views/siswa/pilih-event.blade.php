<x-guest-layout>
    <div class="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
        <div class="w-full max-w-4xl bg-white p-8 rounded-lg shadow-md">
            <h1 class="text-2xl font-bold text-center text-gray-800 mb-2">Pilih Tryout Event</h1>
            <p class="text-center text-gray-600 mb-8">Ujian berikut hanya dapat diakses sesuai jadwal yang ditentukan. Jenjang: <span class="font-semibold">{{ $jenjang }}</span>.</p>
            ...
            <div class="space-y-4">
                @forelse ($paketTryouts as $paket)
                    <div x-data="countdownTimer({{ $paket->waktu_mulai_timestamp ?? 'null' }}, {{ $paket->waktu_selesai_timestamp ?? 'null' }}, {{ $paket->server_now_timestamp ?? 'null' }})"
                         ...
                         :class="{
                             'border-gray-200 hover:border-blue-500': status === 'Sedang Berlangsung',
                             'border-gray-200 opacity-70': status === 'Akan Datang' || status === 'Telah Selesai',
                             'cursor-pointer': status === 'Sedang Berlangsung' || status === 'Akan Datang',
                             'cursor-not-allowed': status === 'Telah Selesai'
                         }">

                        <a href="{{ $paket->event_status == 'Telah Selesai' ? '#' : route('siswa.ujian.mulai', $paket->id) }}"
                           class="group"
                           :class="{'pointer-events-none': status === 'Telah Selesai' || status === 'Akan Datang'}">

                            <div class="flex items-start justify-between">
                                <div>
                                    <h4 class="text-xl font-bold text-gray-800" :class="{'group-hover:text-blue-600': status === 'Sedang Berlangsung'}">
                                        {{ $paket->nama_paket }}
                                    </h4>
                                ...
                                <span class="px-2 py-1 text-xs font-semibold leading-tight rounded-full flex-shrink-0"
                                      :class="{
                                          'text-green-700 bg-green-100': status === 'Sedang Berlangsung',
                                          'text-blue-700 bg-blue-100': status === 'Akan Datang',
                                          'text-gray-700 bg-gray-100': status === 'Telah Selesai'
                                      }" x-text="status">
                                </span>
                            </div>
                            ...
                            <p x-show="countdownText" class="text-sm font-semibold mt-2"
                               :class="{
                                   ...
                                   'text-red-600': status === 'Sedang Berlangsung'
                ...
                @empty
                    <div class="col-span-full text-center py-10 text-gray-500 border rounded-lg">
                        Tidak ada Tryout Event yang tersedia untuk jenjang ini.
                    </div>
                @endforelse
            </div>
