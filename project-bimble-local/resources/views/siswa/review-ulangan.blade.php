                            ...
                            <div class="p-4 bg-gray-700 rounded-lg">
                                <div class="flex items-start space-x-4">
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        {{-- Ambil jawaban siswa untuk soal ini dari variabel $jawabanSiswa --}}
                                        @php
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($soal->pilihanJawaban as $pilihan)
                                                @php
                                                    ...
                                                    if ($isJawabanBenar) {
                                                        $bgColorClass = 'bg-green-800 border-2 border-green-500'; // Jawaban Benar
                                                    } elseif ($isJawabanSiswa && !$isJawabanBenar) {
                                                        $bgColorClass = 'bg-red-800 border-2 border-red-500'; // Jawaban Siswa (Salah)
                                                    }
                                                @endphp

                                                <div class="flex items-center p-3 rounded-lg {{ $bgColorClass }}">
                                                    {{-- Tampilkan ikon berdasarkan kondisi --}}
                                                    <div class="w-6 mr-3 text-center">
                                                        @if ($isJawabanBenar)
                                                            <span class="text-green-400 font-bold">✔</span> {{-- Centang untuk jawaban benar --}}
                                                        @elseif ($isJawabanSiswa && !$isJawabanBenar)
                                                            <span class="text-red-400 font-bold">✖</span> {{-- Silang untuk jawaban salah --}}
                                                        @else
                                                            <span class="text-gray-600 font-bold">&nbsp;</span> {{-- Kosong untuk pilihan netral --}}
                                                        @endif
                                                    </div>
                                                    <div class="prose prose-invert max-w-none text-white">{!! $pilihan->pilihan_teks !!}</div>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
