    ...
    public function update(Request $request, PaketTryout $paketTryout)
{
    ...
    {
        if ($paketTryout->status == 'published') {
        return redirect()->route('paket-tryout.index', ['jenjang' => $paketTryout->mataPelajaran->first()->jenjang_pendidikan ?? null])
            ->with('error', 'Gagal! Paket tryout yang sudah dipublikasikan tidak dapat dihapus. Ubah statusnya ke "draft" terlebih dahulu.');
    }
