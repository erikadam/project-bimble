    ...
    public function showSoal(Request $request, PaketTryout $paketTryout, $mapelId)
{
    ...
    // --- AWAL LOGIKA BARU YANG MENYATUKAN SEMUA JENIS TRYOUT ---
    // Logika ini sekarang berlaku untuk SEMUA jenis tryout (Fleksibel dan Event)

    // 1. Ambil mapel yang sudah dipilih siswa dari session.
    $mapelPilihan = $ujianSiswa['mapel_pilihan'];
    if (empty($mapelPilihan) || !in_array($mapelId, $mapelPilihan)) {
        // Jika siswa mencoba mengakses mapel yang tidak mereka pilih, kembalikan ke halaman pemilihan.
        return redirect()->route('siswa.ujian.pilih_mapel', $paketTryout->id);
    }

    // 2. Ambil detail mapel dan durasinya dari pivot table.
    $mapel = MataPelajaran::findOrFail($mapelId);
    $durasi = $paketTryout->mataPelajaran->firstWhere('id', $mapelId)->pivot->durasi_menit;

    // 3. Atur timer per mata pelajaran, bukan timer global event.
   ...
   public function simpanJawaban(Request $request, PaketTryout $paketTryout, $mapelId)
{
    ...
    // Loop untuk menyimpan atau memperbarui jawaban siswa
    foreach ($semuaSoalDiMapel as $soal) {
        ...
        if (!empty($jawabanPeserta)) {
            if ($soal->tipe_soal === 'isian') {
             ...
             // [DITAMBAHKAN] Logika untuk Pilihan Ganda Kompleks
            elseif ($soal->tipe_soal === 'pilihan_ganda_kompleks') {
                 ...
                 $jawabanPesertaArr = is_array($jawabanPeserta) ? $jawabanPeserta : [];
                 $jawabanUntukDisimpan = json_encode($jawabanPesertaArr);
                 $apakahBenar = (count(array_diff($jawabanBenarDb, $jawabanPesertaArr)) == 0 && count(array_diff($jawabanPesertaArr, $jawabanBenarDb)) == 0);
            }
        }
    ...
    // --- AWAL LOGIKA BARU UNTUK NAVIGASI ---
    // Logika ini disederhanakan dan berlaku untuk semua jenis tryout.

    // 1. Ambil daftar mapel yang dipilih siswa.
    $mapelPilihan = Session::get('ujian_siswa.mapel_pilihan', []);

    // 2. Cari index dari mapel yang baru saja diselesaikan.
    $mapelSekarangIndex = array_search($mapelId, $mapelPilihan);

    // 3. Tentukan mapel selanjutnya.
    $mapelSelanjutnyaId = $mapelPilihan[$mapelSekarangIndex + 1] ?? null;

    // 4. Jika ada mapel selanjutnya, arahkan ke sana.
    ...
    // --- AKHIR LOGIKA BARU UNTUK NAVIGASI ---

    // Jika tidak ada mapel lagi, update total waktu dan arahkan ke halaman hasil.
